...
function wp_head() {
  if (cropping_active(WIDGET_BASE_ID,__NAMESPACE__.'\Widget')) {
    ...
    <style type="text/css">
      ...
      .cat-post-widget ul li.cat-post-item .thumb-wrapper .thumb-color > img {
        -webkit-filter: grayscale(100%);
        filter:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' ><filter id='gray'><feColorMatrix in='SourceGraphic' type='saturate' values='0' /></filter></svg>#gray");
        filter:grayscale(100%);
      }
      .cat-post-widget ul li.cat-post-item:hover .thumb-wrapper .thumb-color > img {
        -webkit-filter: none;
        filter: none;
      }
    </style>
...
class Widget extends \WP_Widget {
  ...
  */
  ...
  function show_thumb($instance) {
    ...
    $featured_image = $featured_images[0]['full'];
    $ret = '';
    ...
    if ( isset( $instance["thumb"] ) && $instance["thumb"] &&
      ...
      if( !(isset( $this->instance['disable_css'] ) && $this->instance['disable_css'])) {
        if( isset($this->instance['thumb_hover'] )) {
          $class = "class=\"cat-post-thumbnail " . $use_css_cropping ." cat-post-" . $instance['thumb_hover'] . " " . $cat . "\"";
        } else {
          $class = "class=\"cat-post-thumbnail " . $use_css_cropping . " " . $cat . "\"";
        }
      }
...
//      $ret .= '<a '.$class . ' href="'.get_the_permalink().'" title="'.the_title_attribute($title_args).'">';
      $ret .= '<div class="thumb-wrapper">';
      $ret .= '<div class="thumb-colorbar"><img src="/wp-content/uploads/2016/11/'.$cat.'.png" alt="" /></div>';
      $ret .= '<div class="thumb-color">' . $this->the_post_thumbnail( array($this->instance['thumb_w'],$this->instance['thumb_h'])) . '</div>';
      $ret .= '</div>';
//      $ret .= '</a>';
    }
  ...
  function itemHTML($instance,$current_post_id) {
    ...
    $ret .= '<a href="'.get_the_permalink().'"><div class="post-item-wrapper">';
...
//      $ret .= '" href="'.get_the_permalink().'" rel="bookmark">'.get_the_title();
  ...
  function widget($args, $instance) {
    ...
    if ( !isset ( $instance["hide_if_empty"] ) || !$instance["hide_if_empty"] || $cat_posts->have_posts() ) {
      ...
      echo "<li class='cat-post-item cat-post-item-empty'></li>";
      echo "<li class='cat-post-item cat-post-item-empty'></li>";
      echo "<li class='cat-post-item cat-post-item-empty'></li>";
