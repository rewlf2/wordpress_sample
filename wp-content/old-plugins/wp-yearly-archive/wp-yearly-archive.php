<?php
/*
    Plugin Name: WordPress Yearly Archive 
    Plugin URI: http://www.prdream.com/wordpress/?page_id=1511
    Description: Displays a monthly or yearly archive of posts. 
    Version: 1.3.0
...
class WP_Yearly_Archive_Widget extends WP_Widget
{
    /**
     * Contstructor.  Set description and call parent class.
     */
    function WP_Yearly_Archive_Widget() 
    {
        /* Widget settings. */
        $widget_ops = array("description" => "Display an archive listing.");

        /* Create the widget. */
        $this->WP_Widget("wp-yearly-archive", "Yearly Archive", $widget_ops);
    }
    ...
    function create_url($interval, $year, $month, $category_id, $category)
    {
        ...
        $url = get_option('home') . "/" . pll_current_language() . "/news/" . $year_month_value;
    ...
    function widget($args, $instance) 
    {
        ...
        if ($display_style == "pulldown") {
            echo "<select name=\"wp-yearly-archive-dropdown\" onchange=\"document.location.href=this.options[this.selectedIndex].value;\">";
    ...
    function form( $instance ) 
    {
        ...
        $defaults = array( "title" => "Yearly Archive", "category_id" => 0, "display_style" => "list", "interval" => "month", "show_counts" => 1 );
    ...
    function register()
    {
        register_widget("WP_Yearly_Archive_Widget");
    }
...
add_action("widgets_init", array('WP_Yearly_Archive_Widget', 'register'));
